package org.example;// напоминалочка получше разобраться в методах 13 и 14
...
public class Main {//коментарии оставляю в большей степени для себя чтоб понимать что я делаю
    public static void main(String[] args) {
        ...
        int[] array3 = createArray(7, 77); // 14+- с помощью великого поисковика
        for (int value : array3) {//14
            System.out.print(value + " ");//14
        }
    }
    ...
    // 14) Создание одномерного массива с длиной len и "размером " ячейки initialValue +
    public static int[] createArray(int len, int initialValue) {
        int[] array = new int[len]; // Создание массива
        for (int i = 0; i < len; i++) {
            array[i] = initialValue; // Заполнение  ячейки -initialValue
        }
